...
if(isset($_GET['id']) && !empty($_GET['id'])){
    ...
    $checkIfArtPanExists = $bdd->prepare('SELECT id, pseudo_acheteur FROM panier WHERE id = ?');
    $checkIfArtPanExists->execute(array($idOfTheArtPan));
    $ArtPanInfos = $checkIfArtPanExists->fetch();

    // Sécurité

    // Sécurity
    if($ArtPanInfos['pseudo_acheteur'] == $_SESSION['pseudo']){
            ...
            $deleteThisCommand = $bdd->prepare('DELETE FROM panier WHERE id = ?');
            $deleteThisCommand->execute(array($idOfTheArtPan));
